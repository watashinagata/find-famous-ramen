...
class Tabelog:
    ...
    """
    def __init__(self, base_url, test_mode=False, p_ward='名古屋市内', begin_page=1, end_page=10):
        ...
        self.url = ''
        self.review = ''
        self.columns = ['store_id', 'store_name','station', 'score', 'ward', 'review_cnt', 'url', 'review']
    ...
    def scrape_item(self, item_url, mode):
        ...
        store_head_list = store_head_list[0].find_all('span')
        print(' 最寄り駅：{}'.format(store_head_list[0].text), end='')
        self.station = store_head_list[0].text
    ...
    def scrape_review(self, review_url):
        ...
        #</div>
        soup = BeautifulSoup(r.content, 'html.parser')
        review_url_list = soup.find_all('div', class_='rvw-item', limit=1) # 口コミ詳細ページURL一覧 #limitでクチコミ件数を変える
    ...
    def make_df(self):
        ...
        se = pd.Series([self.store_id, self.store_name, self.station, self.score, self.ward, self.review_cnt, self.url, self.review], self.columns) # 行を作成
